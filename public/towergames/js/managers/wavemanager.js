...
var TOTAL_WAVES = parseInt(params.get('waves')) || 4;
var nextScene = 'scene' + (TOTAL_WAVES + 1);

console.log(TOTAL_WAVES);

function chanceRoll(p) { return Math.random() * 100 < p; }
...
WaveManager.prototype.update = function(){
	...
	if(this._state == this.states.PAUSED){
		...
		if(CollisionManager.groups.baddies.length < wave.moment){
			...
			if( baddies.length == 0 && CollisionManager.groups.baddies.length == 0){
				...
				if(Date.now() > this.pauseTime){
					...
					if(this.currentWave == TOTAL_WAVES){
						...
						try {
							localStorage.setItem('level1Score', InventoryManager.points);
							if (typeof this.accuracy !== 'undefined') {
								localStorage.setItem('level1Accuracy', this.accuracy);
							}
			...
			// Check if it's the second wave and initiate scene change
...
WaveManager.prototype.getSpawn = function(){
	...
	//50% chance
	var topbottom = chanceRoll(50);

	if(topbottom){

		var top = chanceRoll(50);
