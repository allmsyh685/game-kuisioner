...
InputManager.prototype.setup = function(){
	...
	// Remove mousewheel event listeners (no longer needed)
	// var self = this;
	...
	// console.log('Mousewheel event listeners added to document');

	// Debounce for toolbar selection
	this._toolbarKeyDebounce = 0;
}
...
InputManager.prototype.update = function(){
	...
	if(GUIManager.menuScreen.visible == false){
		...
		// --- Toolbar selection with up/down arrow keys ---
		if (!this._toolbarKeyDebounce || this.game.time.now > this._toolbarKeyDebounce) {
			if (this.game.keys.TOOLBAR_DOWN.isDown) {
				...
				this._toolbarKeyDebounce = this.game.time.now + 200;
			} else if (this.game.keys.TOOLBAR_UP.isDown) {
				if(GUIManager.toolbar.selectedSlot == 0){
					GUIManager.toolbar.selectSlot(GUIManager.toolbar.slots.length - 1);
				} else {
					GUIManager.toolbar.selectSlot(GUIManager.toolbar.selectedSlot - 1);
				}
				this._toolbarKeyDebounce = this.game.time.now + 200;
