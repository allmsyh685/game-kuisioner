...
MainGame.GameState.prototype = {
	...
	create: function(){
		...
		// Set collision on both layers - ground layer (0) and walls layer (1)
		// Ground layer collision tiles: 2, 4, 5, 6 (floor tiles that should block movement)
		this.game.map.setCollision([2, 4, 5, 6], true, 0);
		// Walls layer collision tiles: 0, 7, 9, 17, 25 (actual wall tiles)
		this.game.map.setCollision([0, 7, 9, 17, 25], true, 1);
		...
		if (this.game.floor) {
			CollisionManager.addObjectToGroup(this.game.floor, 'layers');
		} else {
			console.warn('GameState: floor layer is null or undefined!');
		}
		if (this.game.walls) {
			CollisionManager.addObjectToGroup(this.game.walls, 'layers');
		} else {
			console.warn('GameState: walls layer is null or undefined!');
		}
		...
		// Ensure no camera zoom is applied
		if (this.game.camera.scale) {
